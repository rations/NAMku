...
DSP_SAMPLE **dsp::tone_stack::BasicNamToneStack::Process(DSP_SAMPLE **inputs,
...
void dsp::tone_stack::BasicNamToneStack::Reset(const double sampleRate,
...
void dsp::tone_stack::BasicNamToneStack::SetParam(const std::string name,
                                                  const double val) {
  if (name == "bass") {
    ...
    recursive_linear_filter::BiquadParams midParams(sampleRate, midFrequency, midQuality,
                                                    midGainDB);
    ...
    recursive_linear_filter::BiquadParams trebleParams(sampleRate, trebleFrequency,
                                                       trebleQuality, trebleGainDB);
